...
if uploaded_file is not None:
    ...
    # -------- (Corrected Block - Part 1): Split target and one-hot encode features --------
    ...
    # One-hot encode (categoricals) on test data; we align to training columns later
    ...
    if st.button("Make Predictions", use_container_width=True):
        with st.spinner(f"Making predictions using {selected_model}..."):
            ...
            if model is not None:
                # -------- (Corrected Block - Part 2): Align columns, scale (if needed), and predict --------
                # Decide expected features from training artifacts
                ...
                # Remove extra columns and enforce correct order
                X_test = X_test.reindex(columns=expected_features, fill_value=0)

                # Predict (scale for LR/KNN only)
                if selected_model in ["Logistic Regression", "K-Nearest Neighbors"] and scaler is not None:
